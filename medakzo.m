function medakzo
% ©©© MICHAEL DIAZ BAUTISTA. 2024
% This program integrates the Medical Akzo Nobel Problem
% by discretizing and applying the classic Crank-Nicolson scheme
% for the fixed initial-boundary conditions
% The entire entry function phi(t) is displayed, given phi(t) = 2 when
% t <= 5 and phi(t) = 0 for larger values of t. You can modify tf directly.
...
m=200; % Number of points from the spatial mesh
h=1/m;

tol=1e-5; % Tolerance agreement (Newton's method)

%%% Start
y0=initial(m,v0,u0); %Initial Conditions

t0=0;  % Time interval. You may change the value of tf here.
tf=5; 

n=1100;  % Number of points from the temporal mesh
tau=(tf-t0)/n; % time step tau

%%% Main Program
MCN=zeros(2*m,n+1); % Start up of the Approximation Matrix
MCN(:,1)=y0;
y=y0;
t=t0;
for j=0:n-1    
    [ier,ytau]=newton(m,h,c,k,t,y,tau,tol);
    if ier==1
        disp('After 10 iterations, Newton.s method does not achieve the tolerance we require')
        return
    end
    t=t+tau;
    y=ytau;  
...
% Graphics
ts=t0:tau:tf; % Temporal mesh
xs=0:h:1; % Spatial mesh
xs=(xs(2:end));

figure()
% Surf representation of the approximations of u(x,t) with contour lines
subplot(2,1,1)
[X,Y] = meshgrid(xs,ts);
sc = surfc(X,Y,MCN(1:m,:)','FaceAlpha', 0.8, 'EdgeColor', 'none');
ax = gca;
c = gray;
%c = flipud(c);
colormap(c);
...
xlabel('x')
ylabel('t')
title(sprintf('u(x,t), m=%d, n=%d',m,n))


% Surf representation of the approximations of v(x,t) with contour lines
subplot(2,1,2)
surfc(xs,ts,MCN(m+1:end,:)','FaceAlpha', 0.7, 'EdgeColor', 'none');
title(sprintf('v(x,t), m=%d, n=%d',m,n))
xlabel('x')
ylabel('t')

% Spatial level curves. Graphs of u(xi,t) for discrete xi. 
% If m is not big enough, change the x values to generate suitable graphs
figure()
...
% Difference between the estimated solution and the referenced for t=20, u0=0
if tf==20 & u0==0 & m==200
    sol=solut; % two rows u,v 
    ycnu=MCN(1:m,end)';
    ycnv=MCN(m+1:end,end)';
    difu=norm(sol(1,:)-ycnu,inf);
    difv=norm(sol(2,:)-ycnv,inf);
    display(fprintf('Diferencia entre solut y CN u(x,20) en norma infinito=%16.7e\n',difu))
    display(fprintf('Diferencia entre solut y CN v(x,20) en norma infinito=%16.7e\n',difv))
end

%%% Rutines
    function y0=initial(m,v0,u0)  % Initial conditions
    y0=zeros(2*m,1);
    for i=1:m
        y0(i)=u0;
        y0(i+m)=v0;
...
function f=derivada(m,h,c,k,t,y) % Derivative function y'=f(t,y)
    alpha=2*(h-1)^3/c^2;
    beta=(h-1)^4/c^2;
     if t<=5  % phi(t) 
        phi=2;
    ...
    f=zeros(2*m,1);
    f(1)=(phi-2*y(1)+y(2))*beta/h^2+alpha*(y(2)-phi)/(2*h)-k*y(1)*y(m+1);
    f(m+1)=-k*y(1)*y(m+1);
    for i=2:m-1
        xi=i*h;
        alpha=2*(xi-1)^3/c^2;
        beta=(xi-1)^4/c^2;
        f(i)=(y(i-1)-2*y(i)+y(i+1))*beta/h^2+alpha*(y(i+1)-y(i-1))/(2*h)-k*y(i)*y(i+m);
        f(i+m)=-k*y(i)*y(i+m);
    end
    f(m)=-k*y(m)*y(2*m);
    f(2*m)=-k*y(m)*y(2*m);
...
function jac=jacob(m,h,c,k,t,y)  % Jf
    alpha=2*(h-1)^3/c^2;
    beta=(h-1)^4/c^2;
    jac=zeros(2*m,2*m);
    jac(1,1)=-2*beta/h^2-k*y(m+1);
    jac(1,2)=beta/h^2+alpha/(2*h);
    jac(1,m+1)=-k*y(1);
    jac(m+1,1)=-k*y(m+1);
    jac(m+1,m+1)=-k*y(1);
    for i=2:m-1
        ...
        alpha=2*(xi-1)^3/c^2;
        beta=(xi-1)^4/c^2;
        jac(i,i-1)=beta/h^2-alpha/(2*h);
        jac(i,i)=-2*beta/h^2-k*y(i+m);
        jac(i,i+1)=beta/h^2+alpha/(2*h);
        jac(i,2*i)=-k*y(i);
        jac(i+m,i+m)=-k*y(i);
        jac(i+m,i)=-k*y(i+m);
...
function [ier,y1]=newton(m,h,c,k,t,y,tau,tol) % Newton Rapshon's Method
iter=0;
y1=y; % Start up

while iter<=10
    % Trapezoidal Rule Scheme
    g=y1-y-tau/2*derivada(m,h,c,k,t,y)-tau/2*derivada(m,h,c,k,t+tau,y1);
    g1=eye(2*m)-tau/2*jacob(m,h,c,k,t+tau,y1);

    incr=g1\g;
    y1=y1-incr; % New choice 

    if norm(incr,inf)<=tol % Stop by tolerance
        ier=0;
        return
    end
    iter=iter+1; 
end 
ier=1; % Problem with the tolerance
end 

function sol=solut  % Referenced solution 
